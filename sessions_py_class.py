...
combined_df = pd.DataFrame(columns=["Version", "Class", "Count"])
...
for json_file in json_files:
    ...
    # Extract the data from the "Class" section of "tests"
    levels_data = json_data["requests"]["sessions.py"]["Class"]

    # Create a DataFrame for the current JSON file
    df = pd.DataFrame(levels_data.items(), columns=["Class", "Count"])
